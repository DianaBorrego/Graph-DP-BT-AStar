...
public class BT<V, E, S> {
	...
	protected Boolean forget(State<V, E> state, E edge) {
		Boolean r = false;
		if (graph.type().equals(Type.All) || graph.type().equals(Type.One))
			return false;
	...
	private void initialGraph() {
		if (this.withGraph)
			this.outGraph = Graphs2.simpleDirectedGraph();
	}
	...
	public void search(State<V, E> state) {
		...
		if (graph.goal().test(actual)) {
			...
			for (E edge : graph.edgesListOf(actual)) {
				if (this.forget(state, edge) || this.stop)
					continue;
				state.forward(edge);
				search(state);
	...
	public Set<S> getSolutions() {
		if (this.solutions == null)
			return Set.of(this.fsolution.apply(this.optimalPath));
	...
	public static class StatePath<V, E> implements State<V, E> {
		...
		public String toString() {
			return String.format("%s,\n%.2f,\n%s", this.actualVertex, this.getAccumulateValue(), this.getPath()
					.getEdgeList().stream().map(e -> e.toString()).collect(Collectors.joining(",", "{", "}")));
		}
	}
}
