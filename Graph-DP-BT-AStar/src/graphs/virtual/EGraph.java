...
public interface EGraph<V, E> extends Graph<V, E> {
	...
	public static <V extends VirtualVertex<V, E, ?>, E extends SimpleEdgeAction<V, ?>> EGraphBuilder<V, E> virtual(
			V startVertex, Predicate<V> goal, PathType pathType, Type type) {
	...
	public static <V, E> EGraphBuilderVirtualMG<V, E> virtualMG(DirectedMultigraph<V, E> graph, V startVertex,
	...
	public static <V, E> GraphPath<V, E> pathG(GraphPath<VirtualVertexG<V, E>, VirtualEdgeG<V, E>> p,
			Graph<V, E> graph) {
		return new GraphWalk<V, E>(graph, p.getStartVertex().vertex(), p.getEndVertex().vertex(),
				p.getVertexList().stream().map(v -> v.vertex()).toList(),
				p.getEdgeList().stream().map(e -> e.action()).toList(), p.getWeight());
	}

	public static <V, E> GraphPath<V, E> pathMG(GraphPath<VirtualVertexMG<V, E>, VirtualEdgeMG<V, E>> p,
			Graph<V, E> graph) {
		return new GraphWalk<V, E>(graph, p.getStartVertex().vertex(), p.getEndVertex().vertex(),
				p.getVertexList().stream().map(v -> v.vertex()).toList(),
				p.getEdgeList().stream().map(e -> e.action()).toList(), p.getWeight());
