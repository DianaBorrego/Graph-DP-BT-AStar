...
/**
 * <p>
 * Implementation of a simple graph Vertices are assumed to be subtypes of
 * VirtualVertex <V,E>. Edges are assumed to be subtypes of SimpleEdge <V>.
 * </p>
 * 
 * <p>
 * The network is immutable so modification operations are not allowed. Query
 * operations of all vertices or all edges are not allowed either. If any of
 * them are invoked, the UnsupportedOperationException is fired.
 * </p>
 * 
...
public class ESimpleVirtualGraph<V extends VirtualVertex<V, E, ?>, E extends SimpleEdgeAction<V, ?>>
	...
	/**
	 * @param vertex is the current vertex
	 * @return weight of the vertex
	 */
	public double getVertexWeight(V vertex) {
		Double r = 0.;
		if (vertexWeight != null)
			r = vertexWeight.apply(vertex);
		return r;
	}

	/**
	 * @param vertex  Current vertex
	 * @param edgeIn  An incoming edge or incident edge at the current vertex. It is null at the initial vertex.
	 * @param edgeOut A outgoing edge or incident edge at the current vertex. Is null is the final vertex.
	 * @return The weight associated with the vertex assuming the two given edges.
	 */
	public double getVertexPassWeight(V vertex, E edgeIn, E edgeOut) {
		Double r = 0.;
		if (vertexPassWeight != null)
			r = vertexPassWeight.apply(vertex, edgeIn, edgeOut);
		return r;
	}
	...
	public Set<E> getAllEdges(V v1, V v2) {
		Set<E> s = new HashSet<>();
		if (v1.isNeighbor(v2))
			s = Set.of(this.getEdge(v1, v2));
		return s;
	}

	/**
	 * @return Set of vertices of the graph which have been made explicit in the constructor.
	 */
	@Override
