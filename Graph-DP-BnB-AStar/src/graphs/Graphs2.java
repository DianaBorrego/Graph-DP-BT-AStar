...
public class Graphs2 {
	...
	public static <V, E> V closestVertex(Graph<V, E> graph, V vertex, Predicate<V> p) {
		return (Graphs.neighborSetOf(graph, vertex)).stream().filter(p)
				.min(Comparator.comparingDouble(v -> weightToVertex(graph, vertex, v))).get();
	}
	...
	public static <V, E> V furthestVertex(Graph<V, E> graph, V vertex, Predicate<V> p) {
		return (Graphs.neighborSetOf(graph, vertex)).stream().filter(p)
				.max(Comparator.comparingDouble(v -> weightToVertex(graph, vertex, v))).get();
	}
	...
	public static <V, E> SimpleDirectedGraph<V, E> toDirectedGraph(SimpleGraph<V, E> graph) {
		SimpleDirectedGraph<V, E> gs = new SimpleDirectedGraph<V, E>(graph.getVertexSupplier(), graph.getEdgeSupplier(),
				true);
		for (V v : graph.vertexSet()) {
			gs.addVertex(v);
		}
	...
	public static <V, E, G extends Graph<V, E>> G subGraphOfVertices(G graph, Predicate<V> pv, Supplier<G> creator) {
	...
	public static <V, E, G extends Graph<V, E>> G explicitCompleteGraph(G graph, Double weight, Supplier<G> creator,
			Supplier<E> edgeCreator, Function<E, Double> edgeWeight) {
	...
	public static <V, E, G extends Graph<V, E>> G sustituteEdge(G graph, E edge, GraphPath<V, E> graphPath) {
		...
		graphPath.getVertexList().stream().forEach(v -> {
	 ...
	 * @param edgeReverse A function that produces an inverse edge with the same
	 *                    weight.
	 * @param sources     Vertices to be sources
	 * @param targets     Vertices to be targets
	 * @return A directed graph where source vertices have no incoming edges and
	 *         sinks have no outgoing edges.
	 */
	public static <V, E> SimpleDirectedWeightedGraph<V, E> toDirectedWeightedGraphFlow(SimpleWeightedGraph<V, E> graph,
			Function<E, E> edgeReverse, Set<V> sources, Set<V> targets) {
